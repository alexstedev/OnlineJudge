...
mongoose.connection.on("open", function () {
  ...
  users.findOne({ username: "alexstedev" }, function (err, user) {
    var curDate = new Date();
    if (!user) {
      const defaultUser = {
        username: "alexstedev",
        email: "alexstedev@gmail.com",
        hash:
      ...
      users.save(defaultUser, function (err) {
        ...
        console.log(
          "[Database Initialization] New admin user 'alexstedev' was created!"
        );
        console.log("[Default Admin] User Name: alexstedev, Password: 111111");
      });
